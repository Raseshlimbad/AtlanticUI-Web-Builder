...
export async function POST(req: Request) {
  try {
    ...
    if (body?.googleIdToken) {
      ...
      if (!googleUser) {
        return Response.json({ msg: "INVALID_GOOGLE_TOKEN", status: false }, { status: 400 });
      }
      ...
      if (existingUser) {
        ...
        return Response.json({ message: "USER_EXISTS", data: existingUser, status: true }, { status: 200 });
      }
      ...
      return Response.json({ message: "USER_CREATED", data: newUser, status: true }, { status: 200 });
    }
    ...
    if (!body || !body.email || !body.password || !body.name || !body.userID || !Array.isArray(body.projects)) {
      return Response.json({ msg: "INVALID_REQUEST", status: false }, { status: 400 });
    }
